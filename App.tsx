import { BookOpen, Github, LayoutGrid, List, Settings } from 'lucide-react';
...
import { updateDocumentMeta, useRouter } from './hooks/useRouter';
import { IPASection, IPASymbol } from './types';

// Default SEO meta
const DEFAULT_TITLE = 'IPA Master - Learn English Pronunciation';
const DEFAULT_DESCRIPTION = 'Master English pronunciation with our interactive IPA (International Phonetic Alphabet) learning tool. Practice vowels, consonants, and diphthongs with audio examples.';


const App: React.FC = () => {
  const { route, goToHome, goToList, goToArticles, goToArticle } = useRouter();
  ...
  // Derive viewMode and selectedArticleId from route
  const viewMode = route.view;
  const selectedArticleId = route.articleId || null;
  ...
  // Update SEO meta based on current route
  useEffect(() => {
    ...
    if (selectedArticleId) {
      ...
      if (article) {
        updateDocumentMeta({
          title: `${article.title} | IPA Master`,
          description: article.description,
          url: `${baseUrl}/articles/${selectedArticleId}`,
          type: 'article',
        });
        return;
      }
  ...
  const playAudio = (e: React.MouseEvent | React.TouchEvent, word: string) => {
    ...
    console.log('[playAudio] 开始播放发音, word:', word);

    // Find the corresponding IPA symbol data
    const symbolData = findSymbolData(word);
    console.log('[playAudio] 查找符号数据:', symbolData ? `找到 - symbol: ${symbolData.symbol}, filename: ${symbolData.filename}` : '未找到');

    // If found and has filename, play the local MP3
    if (symbolData?.filename) {
      try {
        const audioPath = `/mp3/${symbolData.filename}.wav`;
        console.log('[playAudio] 尝试播放本地音频:', audioPath);
        const audio = new Audio(audioPath);
        const playPromise = audio.play();
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('[playAudio] 音频播放成功');
            })
            .catch((error) => {
              console.warn('[playAudio] 音频播放失败, 回退到TTS:', error.message);
              // Fallback to browser TTS on error
              const utterance = new SpeechSynthesisUtterance(word);
              utterance.lang = 'en-GB';
              utterance.rate = 0.9;
              window.speechSynthesis.speak(utterance);
              console.log('[playAudio] TTS播放已启动');
            });
        }
        return;
      } catch (error) {
        console.error('[playAudio] 播放异常:', error);
        // Fallback on exception
      }
    }

    // Fallback to browser TTS
    console.log('[playAudio] 使用浏览器TTS播放:', word);
    const utterance = new SpeechSynthesisUtterance(word);
    utterance.lang = 'en-GB';
    utterance.rate = 0.9;
    window.speechSynthesis.speak(utterance);
              ...
              <nav className="flex gap-1">
                {(['Vowels', 'Diphthongs', 'Consonants'] as IPASection[]).map((section) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all ${activeSection === section
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200">
              ...
              <button
                ...
                className={`p-1.5 rounded-md transition-all ${viewMode === 'articles' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'
                  }`}
                title="Articles"
              >
                <BookOpen size={18} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1">
        {viewMode === 'grid' ? (
              ...
              {activeSection === 'Consonants' && (
                <div className="flex items-center bg-white border border-slate-200 rounded-lg p-1 shadow-sm">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium flex items-center gap-2 transition-all ${groupingMode === 'manner'
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium flex items-center gap-2 transition-all ${groupingMode === 'voicing'
                        ? 'bg-indigo-50 text-indigo-700'
                        : 'text-slate-500 hover:text-slate-700'
                      }`}
                  >
          ...
          selectedArticleId && getArticleById(selectedArticleId) ? (
            ...
            <ArticleList onSelectArticle={goToArticle} />
          )
        )}
      </main>
